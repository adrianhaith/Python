...
class SkittlesLearner:
    ...
    def select_action(self):
        ...
        dist = multivariate_normal(mean=self.mu, cov = cov)
        a_norm = dist.rvs()
    ...
    def update(self, a_real_rad, reward):
        ...
        a_real_deg = np.array([np.rad2deg(a_real_rad[0]), a_real_rad[1]])
        a_norm = self._to_normalized(a_real_deg)
       
        print("analytical a norm",a_norm)

        # --- Mean update (in normalized space) ---
        cov = self._covariance_norm()
        self.mu_grad = np.linalg.inv(cov) @ (a_norm - self.mu)
        self.mu += self.alpha * (reward - self.rwd_baseline) * self.mu_grad

        # --- ν update ---
        z = self.Q.T @ (a_norm - self.mu)
        lambdas = np.exp(self.nu)
        self.nu_grad = 0.5 * (-1 + (z ** 2) / lambdas)
        self.nu += self.alpha_nu * (reward - self.rwd_baseline) * self.nu_grad
        ...
        self.phi_grad = np.trace(grad_logp_dQ.T @ dQ_dphi)
        self.phi += self.alpha_phi * (reward - self.rwd_baseline) * self.phi_grad
        self.Q = self._rotation_matrix(self.phi)
    ...
    def log_prob(self, a_real_rad):
        # Convert action to degrees and normalize
        a_real_deg = np.array([np.rad2deg(a_real_rad[0]), a_real_rad[1]])
        a_norm = self._to_normalized(a_real_deg)

        # build distribution and compute log probability
        cov = self._covariance_norm()
        dist = multivariate_normal(mean=self.mu, cov = cov)
        return dist.logpdf(a_norm)

def make_history_dict():
    return {
        "action": [],
        "reward": [],
        "mu": [],
